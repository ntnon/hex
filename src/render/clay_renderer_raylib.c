...
void
Clay_Raylib_Initialize (int width, int height, const char *title,
                        unsigned int flags)
{
...
// A MALLOC'd buffer, that we keep modifying inorder to save from so many
// Malloc and Free Calls. Call Clay_Raylib_Close() to free
...
void
Clay_Raylib_Render (Clay_RenderCommandArray renderCommands, Font *fonts)
{
                  ...
                  Ray positionRay = GetScreenToWorldPointWithZDistance (
                      (Vector2){ renderCommand->boundingBox.x
                                 ...
                                 renderCommand->boundingBox.y
                                     + (renderCommand->boundingBox.height / 2)
                                     + 20 },
