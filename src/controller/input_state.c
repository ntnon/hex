...
void get_input_state(input_state_t *out) {
    ...
    if (has_drag_bounds) {
        ...
        if (!mouse_in_drag_bounds) {
            ...
            out->mouse_dragging = out->mouse_right_down &&
                                  (fabsf(out->mouse_delta.x) > drag_threshold ||
    ...
    if (left_button_was_pressed && out->mouse_left_down) {
        // Only check drag bounds if they are set, otherwise allow dragging
        // anywhere
        bool can_drag = !has_drag_bounds || mouse_in_drag_bounds;

        if (can_drag) {
            float delta_magnitude =
              sqrtf(out->mouse_delta.x * out->mouse_delta.x +
    ...
    // Should place tile when: left button released AND wasn't dragging
    out->should_place_tile =
      out->mouse_left_released && !out->mouse_left_was_dragging;
    // --- Modifier keys ---
