...
//
// Coordinate system definitions for each grid type
//

// Simple cartesian coordinates for square grids
typedef struct {
    int x;
    int y;
} square_coord_t;
...
// Example coordinate system for triangular grids
typedef struct {
    int u;
    int v;
    int w;
...
//
// Layout and Pixel Conversion structures
//

// Represents a 2D point, typically for pixel coordinates
typedef struct {
    double x;
    double y;
} point_t;
...
// Holds all layout information for a grid needed for coordinate conversion
typedef struct {
