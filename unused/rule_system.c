...
//       memcpy(cache->cells[range], temp_cells, copy_count *
//       sizeof(grid_cell_t)); cache->counts[range] = copy_count;
//     }
...
//     // Initialize with dummy center - will be set properly when tile is
//     placed grid_cell_t dummy_center = {.type = GRID_TYPE_HEXAGON, .coord.hex
//     = {0, 0}}; spatial_cache_init(&registry->tile_data[i].spatial_cache,
//     dummy_center);
//   }

//   // Initialize scope-based rule arrays
//   registry->self_rules = calloc(registry->rule_capacity, sizeof(uint32_t));
//   registry->neighbor_rules = calloc(registry->rule_capacity,
//   sizeof(uint32_t)); registry->range_rules = calloc(registry->rule_capacity,
//   sizeof(uint32_t)); registry->pool_rules = calloc(registry->rule_capacity,
//   sizeof(uint32_t)); registry->global_rules = calloc(registry->rule_capacity,
//   sizeof(uint32_t));
...
//     rule_t *new_rules = realloc(registry->rules, new_capacity *
//     sizeof(rule_t)); if (!new_rules) {
//       return false;
//     }
...
// uint32_t rule_registry_add_rule(rule_registry_t *registry, const rule_t
// *rule) {
//   if (!registry || !rule) {
//     return 0;
//   }
...
//       return false;
//     return context->current_tile->data.type ==
//     rule->condition_params.tile_type;
...
//                                   rule_context_t *context, const tile_t
//                                   *tile) {
//   if (!registry || !context || !tile) {
...
//   // Check cache (note: this doesn't account for observer position yet -
//   TODO) if (!tile_data->type_dirty &&
//       tile_data->cache_generation == registry->cache_generation) {
...
// void rule_registry_mark_area_dirty(rule_registry_t *registry, grid_cell_t
// cell,
//                                    uint8_t radius) {
...
//     uint32_t tile_index =
//       cell_to_tile_index(cells_in_area[i], 100); // TODO: Get real board
//       width
...
//                                  rule_context_t *context,
//                                  grid_cell_t center_cell, tile_type_t
//                                  tile_type, uint8_t range, tile_t
//                                  **out_tiles, uint32_t max_tiles) {
...
//     spatial_cache_t *cache =
//     &registry->tile_data[center_index].spatial_cache;
...
//                                   tile_type_t target_type, int8_t
//                                   range_delta) {
//   rule_t rule = {0};

//   rule.priority = RULE_PRIORITY_RANGE_MODIFY;
//   rule.scope =
//     (target_type == TILE_UNDEFINED) ? RULE_SCOPE_SELF :
//     RULE_SCOPE_TYPE_GLOBAL;
...
//   // Check scope counts
//   uint32_t total_scope_count = registry->self_count +
//   registry->neighbor_count +
