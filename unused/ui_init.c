...
static Clay_Dimensions SafeMeasureText(Clay_StringSlice text,
    ...
    if (!config || !userData || config->fontId >= MAX_FONTS ||
        ...
        return (Clay_Dimensions){
          .width = (text.length > 0 ? text.length : 10) * 8, .height = 16};
    }
    ...
    Clay_TextElementConfig safeConfig = {
      ...
      .fontId = safeFontId,
      .fontSize = (config->fontSize > 0 && config->fontSize <= 72)
                    ? config->fontSize
                    : 16,
      .letterSpacing =
        (config->letterSpacing >= 0 && config->letterSpacing <= 10)
    ...
    return Raylib_MeasureText(text, &safeConfig, userData);
}
...
UI_Context ui_init(int screen_width, int screen_height) {
    ...
    for (int i = 0; i < MAX_FONTS; i++) {
        if (UI_FONTS[i].glyphs != NULL) {
            ...
            if (glyphAddr % 8 != 0) {
                printf("Warning: Font %d glyph data is not 8-byte aligned "
                       "(addr: %p)\n",
                       i, UI_FONTS[i].glyphs);
            }
        }
